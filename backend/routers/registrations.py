...
from lib.event_config import GAMES, PAYMENT, ORGANIZER_EMAIL, REGISTRATION_ID_PREFIX, fee_for, format_fee, game_title
...
async def registration_config() -> RegistrationConfig:
    return RegistrationConfig(
        games=[GameConfig(id=gid, title=title, fee=fee_for(gid), fee_display=format_fee(fee_for(gid))) for gid, title in GAMES.items()],
...
async def submit_registration(
    ...
    try:
        data = RegistrationInput(
            ...
            student_id=student_id, game=game, utr_number=utr_number,  # type: ignore[arg-type]
        )
    ...
    dup = await db.registrations.find_one(
        {"game": data.game, "registration_status": {"$ne": "REJECTED"}, "$or": [{"email": data.email}, {"mobile": data.mobile}]},
