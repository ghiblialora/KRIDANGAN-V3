...
// Multipart upload (files + fields). The browser sets the multipart boundary header itself.
...
// Human-readable message out of an ApiError body (string detail or FastAPI validation list).
export function errorMessage(err: unknown, fallback = "Something went wrong. Please try again."): string {
  if (err instanceof ApiError) {
    ...
    if (err.status === 413) return "File is too large (max 5 MB).";
